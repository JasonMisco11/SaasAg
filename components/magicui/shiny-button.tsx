...
const ShinyButton = ({ children, className }: { children: ReactNode; className?: string }) => {
  return (
    <motion.button
      {...animationProps}
      className={cn(
    ...
    >
      <span
        className={cn(
          "relative block h-full w-full text-sm uppercase tracking-wide text-[rgb(0,0,0,65%)] dark:font-light dark:text-[rgb(255,255,255,90%)]",
          className
        )}
        style={{
          maskImage:
